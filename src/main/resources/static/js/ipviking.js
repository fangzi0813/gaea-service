    ...
    */
    ...
    function start(loc, psk) {
        ...
        webSocket.onmessage = function (evt) {
            ...
            //alert(d3.select("#Xmpp"));

            // Parse the json to a js obj and clean the data
            var datum = eval("(" + evt.data + ")");
            var startLoc, endLoc;
            if (datum.from && datum.to) {
                var start = d3.select("#" + datum.from);
                var startX = Number(start.attr("xx"));
                var startY = Number(start.attr("yy"));
                startLoc = [startX, startY];

                var end = d3.select("#" + datum.to);
                var endX = Number(end.attr("xx"));
                var endY = Number(end.attr("yy"));
                endLoc = [endX, endY];
            } else {
                ...
                startLoc = projection([datum.longitude, datum.latitude]);
                endLoc = projection([datum.longitude2, datum.latitude2]);
            ...
            datum.service = undefined; // datum.dport in ports ? ports[datum.dport] : undefined;
            datum.cx = startLoc[0];
            datum.cy = startLoc[1];
            datum.x = startLoc[0];
            datum.y = startLoc[1];
    ...
    queue()
        .defer(d3.json, "res/js/china_json")
        .defer(d3.tsv, "res/js/port-names_tsv")
        //.defer(d3.csv, "res/js/country-codes_csv")
    ...
    d3.select('#switch-map-control, #switch-map-control > svg').on('mouseover', function () {
        d3.select('#switch-maps').style('fill', '#00a6ff');
    }).on('mouseout', function () {
        d3.select('#switch-maps').style('fill', '#fff');
    }).on('click', function () {
        // FIXME this needs to be an argument
        // should have check.js load, check qs args, then dynamically load the correct javascript
        window.location.href = '/';
        // d3.xhr('/', function (err, res) {
        //     if (err || !res || res.status !== 200) window.location.href = 'http://map.norsecorp.com';
        //     // FIXME doesn't clear external javascript
