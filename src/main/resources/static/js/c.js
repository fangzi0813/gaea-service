window.SINGLE_TAB = "  ";
window.ImgCollapsed = "res/img/Collapsed_gif";
window.ImgExpanded = "res/img/Expanded_gif";
...
function Process(id, to) {
    ...
    try {
        ...
        $id(to).innerHTML = "<PRE class='CodeContainer'>" + html + "</PRE>";
    } catch (e) {
        //alert("JSON数据格式不正确:\n"+e.message);
        $id(to).innerHTML = json;
    }
}
...
function CollapseLevel(level,id,to) {

    EnsureIsPopulated(id,to);
    TraverseChildren($id(to), function (element, depth) {
        if (element.className == 'collapsible') {
            if (depth >= level) {
                MakeContentVisible(element, false);
            } else {
...
function EnsureIsPopulated(id,to) {

    if (!$id(to).innerHTML && !!$id(id).value) Process();
